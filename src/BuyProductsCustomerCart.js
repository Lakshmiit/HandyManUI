...
const NotificationsList = ({ notifications, highlightedItem }) => {
  ...
  const customerOrdersNotifications = notifications.filter(
    (item) => item.assignedTo === "Customer Care" && item.status === "Draft"
  ); 
  
//   const handleOrdersClick = ( buyProductId) => {
    
//     navigate(`/buyProductsCartView/${customerId}/${buyProductId}/${userType}`, { state: { buyProductId } });
//   };
    ...
    const fetchNotifications = async () => {
      try {
        const [buyProductResponse] = await Promise.all([
          fetch(
          `https://handymanapiv2.azurewebsites.net/api/BuyProduct/GetBuyProductDetailsForAdmin`
        ),
      ]);
...
export default BuyProductsCustomerCart;
