...
const ProductView = () => {
  ...
  useEffect(() => {
    setLoading(true);
    const url = `https://localhost:7091/api/Product/GetProductList?ProductOwnedBy=Admin`
    axios.get(url)
      .then(response => {
  ...
  {currentProducts.map((product, index) => (
    <tr
      key={index}
      style={{
        backgroundColor: product.productStatus === 'Approve' ? '#d4edda' : 'transparent', // Green background for Approved
      }}
    >
