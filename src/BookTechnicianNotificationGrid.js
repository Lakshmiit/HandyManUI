...
const BookTechnicianNotification = () => {
  ...
  useEffect(() => {
    setLoading(true);
    const url = `https://lmartzoneav1-bhdzfxcse7ctdxbd.westus2-01.azurewebsites.net/api/BookTechnician/GetBookTechnicianForAdminList`
    axios.get(url)
      .then(response => {
  ...
  const handleDelete = (technicianId) => {
    ...
    if (confirmDelete) {
      axios.delete(`https://lmartzoneav1-bhdzfxcse7ctdxbd.westus2-01.azurewebsites.net/api/RaiseTicket/${technicianId}`)
        .then(() => {
