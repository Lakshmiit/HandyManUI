...
useEffect(() => {
  const fetchCollectionData = async () => {
    try {
      ...
      const url = `https://lmartzoneav1-bhdzfxcse7ctdxbd.westus2-01.azurewebsites.net/api/UploadLakshmiCollection/GetAllLakshmiCollectionsByCategory?category=${encodedCategory}`;
      ...
      approvedList.forEach(async (product) => {
        ...
        try {
          const res = await fetch(
            `https://lmartzoneav1-bhdzfxcse7ctdxbd.westus2-01.azurewebsites.net/api/FileUpload/download?generatedfilename=${encodeURIComponent(firstPhoto)}`
          );
          let blob;
