...
const AartmentRaiseTicketNotification = () => {
  ...
  useEffect(() => {
    setLoading(true);
    const url = `https://handymanapiv13-c7gjhhefa2bfgedb.centralindia-01.azurewebsites.net/api/ApartmentRaiseTicket/GetGetApartmentMaintenanceForAdminList`
  ...
  const handleDelete = (ticketId) => {
    ...
    if (confirmDelete) {
      axios.delete(`https://handymanapiv13-c7gjhhefa2bfgedb.centralindia-01.azurewebsites.net/api/RaiseTicket/${ticketId}`)
        .then(() => {
  ...
  return (
    ...
    <div className="d-flex flex-row justify-content-start align-items-start">
      ...
      <div className={`container m-1 ${isMobile ? "w-100" : "w-75"}`}>
        <h2 className="text-center mb-4">Apartment Raise a Ticket Notifications</h2>
        ...
        <table className="table table-bordered">
          ...
          <tbody>
            {currentRaiseTicket.map((ticket, index) => (
              <tr key={index}>
                <td>{ticket.userId}</td>
                <td>{ticket.apartmentRaiseTicketId}</td>
                ...
                <td className="d-flex align-items-center">
                  <Link
                    to={`/apartmentRaiseTicketActionView/${ticket.id}`}
  ...
  {currentRaiseTicket.map((ticket, index) => (
    <div key={index} className="ticket-card">
      ...
      <div className="ticket-actions">
        <Link to={`/apartmentRaiseTicketActionView/${ticket.id}`} className="btn btn-info mx-2">
          <FaEye />
        </Link>
...
export default AartmentRaiseTicketNotification;
