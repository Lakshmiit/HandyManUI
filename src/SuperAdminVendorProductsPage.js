...
//       const photo = Array.isArray(item.images) ? item.images[0] : null;
//       if (!photo) return;
//       try {
//         const filename = getImageFilename(photo);
//         if (!filename) {
//           const directUrl = imageValueToUrl(photo);
//           if (directUrl && !cancelled)
//             setRecentImageUrls((prev) => ({ ...prev, [item.id]: directUrl }));
//           return;
//         }
//         let imageData = await ImageCache.getBase64(filename);
//         if (!imageData) {
...
//         }
//         if (!cancelled) {
//           setRecentImageUrls((prev) => ({
//             ...prev,
//             [item.id]: `data:image/jpeg;base64,${imageData}`,
//           }));
//         }
//       } catch {}
//     };

//     Promise.allSettled(recentProducts.map(fetchOne));
