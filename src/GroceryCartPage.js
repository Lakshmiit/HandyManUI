...
const GroceryCartPage = () => {
  ...
  const [selectedVendorId] = useState(
        ...
        const results = await Promise.allSettled(
          filenames.map(async (fn) => {
            ...
            if (contentType.includes("application/json")) {
              ...
              return {
                fn,
                url: `https://apiqa-b5cyfzbhhah5adc9.westus2-01.azurewebsites.net/api/FileUpload/download?generatedfilename=${encodeURIComponent(fn)}`,
              };
            }
          }),
        );
