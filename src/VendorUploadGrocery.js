...
const VendorUploadGrocery = () => {
  ...
  const selectedPincode = selectedZipCodes.map((item) => item.value).join(",");
  ...
  const loadVendorDetails = async () => {
    try {
      ...
      const response = await fetch(
        `https://apiqa-b5cyfzbhhah5adc9.westus2-01.azurewebsites.net/api/VendorRegistration/GetVendorDetailsByvendorId?vendorId=${vendorId}`,
      );
      ...
      // Convert zip codes into MultiSelect options
      const zipOptions = vendor.zipCode.split(",").map((zip) => ({
        label: zip.trim(),
        value: zip.trim(),
      }));
      ...
      setZipCodes(zipOptions);

      // Optional: preselect all zip codes
      setSelectedZipCodes(zipOptions);
    } catch (error) {
      console.error(error);
    }
  };
  ...
  const handleSubmit = async (event) => {
    ...
    const payload = {
      ...
      storeName: storeName,
      selectedZipcodes: selectedZipCodes.map((item) => item.value).join(","),
    };
    try {
      const response = await fetch(
        `https://apiqa-b5cyfzbhhah5adc9.westus2-01.azurewebsites.net/api/UploadGrocery/UploadGrocery`,
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        navigate(`/VendorGroceryList/${vendorId}`);
      } else {
  ...
  return (
    <div className="d-flex flex-row justify-content-center align-items-center mt-mob-50">
      {/* Sidebar */}
      {/* {!isMobile && (
      ...
      <div className={`container m-3 ${isMobile ? "w-100" : "w-75"}`}>
        ...
        <div className="bg-white rounded-3 p-3 bx_sdw w-60 m-auto">
          <form onSubmit={handleSubmit}>
            ...
            <div className="form-group">
              <label>
                Store Name <span className="req_star">*</span>
              </label>
              <input
                type="text"
                className="form-control"
            ...
            <div className="form-group">
              <label>
                Select PinCode <span className="req_star">*</span>
              </label>
              ...
              <MultiSelect
                ...
                labelledBy="Select PinCode"
                overrideStrings={{
                  selectSomeItems: "Select PinCode",
                }}
              />
            </div>
            ...
            <div className="d-flex justify-content-between gap-3 mt-3">
              ...
              <button
                ...
                onClick={() => navigate(`/VendorGroceryList/${vendorId}`)}
              >
...
export default VendorUploadGrocery;
