...
const AdminLakshmiCollectionsApproval = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`https://lmartzoneav1-bhdzfxcse7ctdxbd.westus2-01.azurewebsites.net/api/UploadLakshmiCollection/GetLakshmiCollections?id=${id}`);
        const data = await response.json();
        setCollectionData(data);
         const imagePromises = (data.images ?? []).map(async (file) => {
          const r = await fetch(
            `https://lmartzoneav1-bhdzfxcse7ctdxbd.westus2-01.azurewebsites.net/api/FileUpload/download?generatedfilename=${encodeURIComponent(file)}`
          );
