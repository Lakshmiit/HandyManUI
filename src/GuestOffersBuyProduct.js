...
const GuestOffersBuyProduct = () => { 
  ...
  useEffect(() => {
    console.log(loading,editingAddressId, submitted, error, productOptions)
  }, [loading, editingAddressId, submitted, error, productOptions])
...
useEffect(() => {
  ...
  //   try {
  //     const response = await fetch(`https://handymanapiv2.azurewebsites.net/api/UserOnBoarding/GuestUserVerificationByMobileNo?mobileNo=${mobileNumber}`);
  ...
  const fetchCustomerDetails = async () => {
    try {
      const response = await fetch(`https://handymanapiv2.azurewebsites.net/api/Customer/GetCustomerDtailsByUserId?userId=${userData.userId}`);
      const data = await response.json();
      console.log("Customer Details:", data);
      setAddresses(data); 
      setGuestCustomerId(data[0]?.id);  
      setUserId(userData.userId);   
  ...
  const handleSendOTP = async (e) => {
    ...
    try {
      ...
      const verifyResponse = await fetch(`https://handymanapiv2.azurewebsites.net/api/UserOnBoarding/GuestUserVerificationByMobileNo?mobileNo=${mobileNumber}`);
  ...
  const handleVerifyOTP = async (e) => {
    ...
    try {
      ...
      alert("OTP Verified Successfully. Please Add Your Address.");
      localStorage.setItem('guestMobileNumber', mobileNumber);
      localStorage.setItem('guestOTP', otp.join(''));
      }    
       catch (error) {
...
const handleGetQuotation = async (e) => {
  ...
  const { address, firstName, mobileNumber, zipCode } = addressData;

  if (!address?.trim() || !firstName?.trim() || !mobileNumber?.trim() || !zipCode?.trim()) {
 ...
 const payload2 = {
   ...
   CustomerPhoneNumber: addressData.mobileNumber,
   category:category,
   status: "Draft",
   productName,
   ...
   CustomerId: userId,
   CustomerName: addressData.firstName,
   RequestedBy: userId,
   PaymentMode:"",
 ...
 try {
   ...
   localStorage.removeItem('guestOTP');
   localStorage.removeItem('guestAddressData'); 
 navigate(`/guestProductPaymentPage/customer/${userId}/${buyProductData.buyProductId}`);
 } catch (error) {
...
useEffect(() => {
  const isFromViewPage = location.state?.fromViewPage;
  if (isFromViewPage) {
    const savedMobileNumber = localStorage.getItem('guestMobileNumber');
    const savedOTP = localStorage.getItem('guestOTP');
    const savedAddress = localStorage.getItem('guestAddressData');
    ...
    if (savedAddress) {
      ...
      setFirstName(parsedAddress.firstName || '');
      setNewAddress(parsedAddress.address || '');
...
const handleBothActions =  (e) => {
  ...
  <Header />
    ...
    <div className="input-group-append">
      <button
        type="button"
        className="btn btn-dark h-100" size="sm"
  ...
  {NumberError && <p style={{ color: "red" }}>{NumberError}</p>}
            ...
            <div className="col-md-6">
                ...
                <div class="otp_input" id="otp_input">
                      ...
                      <button
                      type="button"
                      className="btn btn-dark" size="sm"
                      onClick={handleVerifyOTP}
                    >
                      Verify
                    </button>
                    {isOTPValid === true && <span className="text-success ms-2">✔️</span>}
{isOTPValid === false && <span className="text-danger ms-2">❌</span>}
                ...
                {/* <div>
                    Incorrect Mobile Number? <a class="link" asp-area="" >Change</a>
                </div> */}
                {timerRunning && (
              ...
              <div className="d-flex justify-content-between align-items-center">
                ...
                <Button variant="success m-1 text-white" onClick={() => setShowModal(true)} disabled={!mobileNumber.trim()}>
                  Add Address
                </Button>
      ...
      {/* Modal */}
        ...
        <Modal.Body>
          <Form>
            ...
            <Button type="button" variant="primary" onClick={isEditing ? handleAddressEdit : handleBothActions}>
              ...
              .map((address) => (
                ...
                >
                  <div>
                    ...
                    <br /> */}
                    <span className="ml-2">{address.firstName}</span>
                    <br />
        ...
        <div class="text-justify">
            ...
            <button
              ...
              <button
                ...
                onClick={() => navigate(`/offersGuest/customer/guest`)}
...
export default GuestOffersBuyProduct;