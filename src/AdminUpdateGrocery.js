...
useEffect(() => {
  ...
  useEffect(() => {     
          const fetchGroceryData = async () => {
              try {
                  setLoading(true);
                  const groceryResponse = await fetch(`https://handymanapiv13-c7gjhhefa2bfgedb.centralindia-01.azurewebsites.net/api/UploadGrocery/GetGroceryItems/${id}`);
...
const handleRemoveFile = (index) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch(`https://handymanapiv13-c7gjhhefa2bfgedb.centralindia-01.azurewebsites.net/api/UploadGrocery/UpdateGroceryItems?id=${uniqueId}`, {
        method: "PUT",
        headers: {
