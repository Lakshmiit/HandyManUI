...
useEffect(() => {
  ...
  useEffect(() => {     
          const fetchGroceryData = async () => {
              try {
                  setLoading(true);
                  const groceryResponse = await fetch(`https://lmartzoneav1-bhdzfxcse7ctdxbd.westus2-01.azurewebsites.net/api/UploadGrocery/GetGroceryItems/${id}`);
...
const handleRemoveFile = (index) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch(`https://lmartzoneav1-bhdzfxcse7ctdxbd.westus2-01.azurewebsites.net/api/UploadGrocery/UpdateGroceryItems?id=${uniqueId}`, {
        method: "PUT",
        headers: {
