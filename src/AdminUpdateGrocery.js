...
useEffect(() => {
  ...
  useEffect(() => {     
          const fetchGroceryData = async () => {
              try {
                  setLoading(true);
                  const groceryResponse = await fetch(`https://lmartapiv1-fxcyd2b4btacgsav.westus2-01.azurewebsites.net/api/UploadGrocery/GetGroceryItems/${id}`);
...
const handleRemoveFile = (index) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch(`https://lmartapiv1-fxcyd2b4btacgsav.westus2-01.azurewebsites.net/api/UploadGrocery/UpdateGroceryItems?id=${uniqueId}`, {
        method: "PUT",
        headers: {
