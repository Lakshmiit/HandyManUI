...
const GroceryCard = () => {
  ...
  useEffect(() => {
    ...
    // ProfilePage already resolved each vendor product entry (productId +
    // discount + qty from vendorlist.json) against the grocery catalog it
    // had already fetched, so these arrive as full product objects — no
  ...
  useEffect(() => {
    ...
    const vendorProducts = Array.isArray(vendorData.products)
      ? vendorData.products
      : [];
    const imageProducts = vendorProducts
      .map((product) => ({
        product,
    ...
    const fetchOne = async ({ product, photo }) => {
      try {
        ...
        // getBase64 is async — this was previously called without await,
        // so `cached` was always a truthy Promise object and every vendor
        // product ended up with a broken "data:image/...;base64,[object
        // Promise]" src instead of its real LMart master-catalog photo.
  ...
  useEffect(() => {
    // Vendor listings are resolved locally from vendorlist.json + the
    // already-fetched catalog (see the effect above) — Lakshmi Mart's own
    // categories are the only ones that should hit the live API.
    if (vendorData?.isVendorGrocery) return;
    if (!encodedCategory) return;
    const decodedCat = decodeURIComponent(encodedCategory);
    setSelectedCategory(decodedCat);
    ...
    const fetchProducts = async () => {
      try {
        ...
        setLoadingImages(
          Object.fromEntries(
            imageProducts.map(({ product }) => [product.id, true]),
          ),
        );
        ...
        await Promise.allSettled(
          imageProducts.map(async ({ product, photo }) => {
            ...
            if (!imageData) {
              ...
              imageData = data?.imageData || "";
              if (!imageData)
                throw new Error("Image response did not contain imageData");
              await ImageCache.setBase64(filename, imageData);
            }
