...
// etc.) was built against for vendorlist.json — normalize once here so
// every caller keeps using vendorId / storeName / categories / category /
// productId / qty, same shape as before. Categories are also sorted by
// whatever "rank" the vendor/admin set (Vendor Preview page / Super Admin
// vendor products page) so every consumer — including the customer-facing
// Profile page — shows them in that same arranged order automatically,
// with unranked categories (older records) falling back to the order the
