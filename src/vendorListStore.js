...
// The endpoint replies with a flat array of the vendor's own approved/
// pending product records (same shape as a grocery catalog item, plus
// `groceryItemId` linking back to the master catalog entry and its own
// per-product `status`) — NOT the nested vendor/categorie shape used by
// GetVendorsByPincode. It may also reply with a single object, or nothing
// at all (204/404/empty body) when the vendor hasn't submitted anything —
// handle all three the same way, grouping products into categories here.
function normalizeVendorProductsResponse(data, vendorId) {
  ...
  list.forEach((p) => {
    if (!p) return;
    const categoryName = p.category || p.categoryName || "Unspecified";
    if (!categoryMap.has(categoryName)) categoryMap.set(categoryName, []);
    categoryMap.get(categoryName).push({
      productId: p.groceryItemId || p.id,
      name: p.name || "",
      images: Array.isArray(p.images) ? p.images : [],
      qty: Number(p.stockLeft ?? p.quantity ?? p.qty ?? 0),
      discount: Number(p.discount || 0),
  ...
  // Overall badge status: Approved only once every product is Approved,
  // otherwise treat the submission as still pending.
  const overallStatus = list.every((p) => p.status === "Approved")
    ? "Approved"
    : "Pending Approval";
...
async function fetchVendorProductsFromServer(vendorId) {
  ...
  try {
    ...
    vendor = normalizeVendorProductsResponse(data, vendorId);
  } catch (err) {
