...
function ReedemCode({
  ...
  useEffect(() => {
    console.log(showCoinsModal);
  }, [showCoinsModal]);
  
  const fetchNumbers = async () => {
    try {
      const response = await fetch(
        "https://handymanapiv14-cvccacc0cbggefds.centralindia-01.azurewebsites.net/api/Customer/download-json"
      );
...
//   useEffect(() => {
//   const lock = localStorage.getItem(`hm_referral_lock_${referrerId || "guest"}`);
//   if (initialOpen && lock !== "true") {
//     setOpen(true);
//   }
// }, [initialOpen, referrerId]);

const existingNumbersSet = useMemo(() => {
  return new Set((UsersData || []).map(num => String(num)));
}, []);

const isInUsersData = useCallback((num) => {
  return existingNumbersSet.has(num);
}, [existingNumbersSet]);
...
const hydrateFromServer = useCallback(async () => {
  //   const lock = localStorage.getItem(`hm_referral_lock_${referrerId || "guest"}`);
  // // ✅ STOP opening popup if already invited
  // if (lock === "true") {
  //   setOpen(false);
  //   return;
  // }
...
useEffect(() => {
  ...
  const handlePhoneChange = async (index, raw) => {
    ...
    if (isInUsersData(value)) {
      nextStatus[index] = "invalid";
      nextErrors[index] = "This number already exists. Enter a NEW number.";
      setStatus(nextStatus);
      setErrors(nextErrors);
      return;
    }
  ...
  const upsertReferral = async ({ record, numbersCsv }) => {
    ...
    if (record?.id) {
      const putUrl = `https://handymanapiv14-cvccacc0cbggefds.centralindia-01.azurewebsites.net/api/ReferralPoints/UpdateReferralPoints?id=${encodeURIComponent(
        record.id
      )}`;
      ...
      const postUrl = `https://handymanapiv14-cvccacc0cbggefds.centralindia-01.azurewebsites.net/api/ReferralPoints/UploadReferralPoints`;
  ...
  const handleRedeem = async () => {
    try {
     ...
     await fetch(`https://handymanapiv14-cvccacc0cbggefds.centralindia-01.azurewebsites.net/api/Auth/sendpromosms`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ mobile: numbersArr.join(","), name: customerName }),
      });
      alert(`🎉 Thank you for referring your friend! ₹100 referral amount has been added to your wallet after they registered.🙏💰✨`);
      localStorage.setItem(`hm_referral_lock_${referrerId || "guest"}`,
      "true"
    );
      closeDialog();
      ...
      onInviteSuccess?.({
        refRecord: {
          referralNumbers: numbersArr.join(","),
        },
        points: 0
      });
      setPolling(true);
  ...
  const interval = setInterval(async () => {
    ...
    const results = await Promise.all(
      refs.map(async (num) => {
        ...
        const res = await checkNewOrExisting(num);
        return res === "exists" ? "registered" : "pending";
      })
    );
    setStatus(results);
    const allRegistered = results.every((s) => s === "registered");
    if (allRegistered) {
      localStorage.setItem(AWARDED_POINTS_KEY, "100");
      // ✅ instead of setClaimAvailable
      onPointsChange?.(100);
      setPolling(false);
      alert("🎉 All 3 users registered! You can claim coins.");
    }
  }, 3000);
  return () => clearInterval(interval);
}, [polling, refs, AWARDED_POINTS_KEY, checkNewOrExisting, onPointsChange]);
