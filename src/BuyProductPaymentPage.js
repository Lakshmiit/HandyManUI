...
useEffect(() => {
  ...
  useEffect(() => {
    const fetchProductData = async () => {
      try {
        const response = await fetch(`https://lmartzoneav1-bhdzfxcse7ctdxbd.westus2-01.azurewebsites.net/api/BuyProduct/GetBuyProductDetailsById/${buyProductId}`);
        if (!response.ok) {
   ...
   useEffect(() => {
            const fetchProductData = async () => {
                try {
                    setLoading(true);
                    const productResponse = await fetch(`https://lmartzoneav1-bhdzfxcse7ctdxbd.westus2-01.azurewebsites.net/api/Product/${id}`);
...
const handleGetQuotation = async (e) => {
  ...
  if (selectedPayment === 'online') {
       response = await fetch(`https://lmartzoneav1-bhdzfxcse7ctdxbd.westus2-01.azurewebsites.net/api/BuyProduct/${buyProductId}`,{
        method: 'PUT',
        headers: {
...
const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch(`https://lmartzoneav1-bhdzfxcse7ctdxbd.westus2-01.azurewebsites.net/api/Product/${uniqueId}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json"
        },
