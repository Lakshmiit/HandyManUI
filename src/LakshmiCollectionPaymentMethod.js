...
const getUploadItemByProductName = useCallback(async (productName) => {
 ...
 const fetchCustomerData = useCallback(async () => {
      try {
        const response = await fetch(`https://lmartapiv1-fxcyd2b4btacgsav.westus2-01.azurewebsites.net/api/Address/GetAddressById/${userId}`);
        if (!response.ok) {
...
const handleUpdatePaymentMethod = async (e) => {
  ...
  try {
      ...
      let response = await fetch(`https://lmartapiv1-fxcyd2b4btacgsav.westus2-01.azurewebsites.net/api/LakshmiCollection/UpdateLakshmiCollectionDetails/${collectionId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
