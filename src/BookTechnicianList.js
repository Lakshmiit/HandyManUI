...
const BookTechnicianList = () => {
  ...
  useEffect(() => {
    setLoading(true);
    const url = `https://handymanwebapp1-ezgyf8bxf4dtcqd2.z01.azurefd.net/api/UploadJobDescriptionBookTechnician/GetUploadJobDescriptionDetails`
    axios.get(url)
      .then(response => {
  ...
  const handleDelete = (technicianId) => {
    ...
    if (confirmDelete) {
      axios.delete(`https://handymanwebapp1-ezgyf8bxf4dtcqd2.z01.azurefd.net/api/UploadJobDescriptionBookTechnician/${technicianId}`)
        .then(() => {
