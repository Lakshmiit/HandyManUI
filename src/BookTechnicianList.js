...
const BookTechnicianList = () => {
  ...
  useEffect(() => {
    setLoading(true);
    const url = `https://lmartzoneav1-bhdzfxcse7ctdxbd.westus2-01.azurewebsites.net/api/UploadJobDescriptionBookTechnician/GetUploadJobDescriptionDetails`
    axios.get(url)
      .then(response => {
  ...
  const handleDelete = (technicianId) => {
    ...
    if (confirmDelete) {
      axios.delete(`https://lmartzoneav1-bhdzfxcse7ctdxbd.westus2-01.azurewebsites.net/api/UploadJobDescriptionBookTechnician/${technicianId}`)
        .then(() => {
