...
//       setItems(normalized);
//       // Submission quantities are a standing selection, not a delta against
//       // live stock, so they deliberately survive a catalog refresh.
//     } catch (err) {
...
//   // Restore any products the vendor already checked/discounted last time
//   // they were on this page, so the cart survives navigation/reloads.
//   useEffect(() => {
...
//           if (p?.productIds) {
//             map[p.productIds] = { checked: true, discount: String(p.discount ?? "0") };
//             qtyMap[p.productIds] = Number(p.quantity || 0);
//           }
//         });
//       });
//       if (Object.keys(map).length) setSelection(map);
//       if (Object.keys(qtyMap).length) setPendingQty((prev) => ({ ...prev, ...qtyMap }));
//     } catch (err) {
...
//   // exact shape vendorUploadProducts expects — every time quantity or
//   // discount changes, no separate "save" step. Dropping a product's
//   // quantity back to 0 drops it out of this payload automatically.
//   useEffect(() => {
//     if (!vendor) return;
//     const selectedItems = items.filter((it) => Number(pendingQty[it.id] || 0) > 0);
//     const categorieMap = {};
...
//     }
//   }, [selection, pendingQty, items, vendor, vendorId]);
...
//           return {
//             ...prevSel,
//             [itemId]: { checked: true, discount: current?.discount ?? String(item?.discount ?? 0) },
//           };
//         }
...
//   const getSelectionDiscount = (item) => selection[item.id]?.discount ?? String(item.discount ?? 0);
...
//                     <img loading="lazy" decoding="async" src={getCategoryImage("All")} alt="All" className="vsu-cat-img" />
...
//                   <span className="badge bg-success">{selectedForSubmissionCount} selected for submission</span>
//                 )}
...
//                         style={{
//                           top: 6, left: 6, zIndex: 3, background: "#fff", borderRadius: "50%",
//                           width: "22px", height: "22px", border: "1px solid rgba(0,0,0,0.08)", cursor: "pointer",
//                         }}
...
//                             onClick={() => restockQty > 0 && handlePendingChange(item.id, -1, item)}
...
//                             type="button"
//                             className="vsu-restock-btn"
//                             onClick={() => handlePendingChange(item.id, 1, item)}
...
//                             <span className="fw-bold" style={{ color: "#2F6B4F" }}>Selected</span>
//                           )}
//                         </div>
...
//                             onChange={(e) => updateSelectionDiscount(item, e.target.value)}
//                           />
...
// { categories: [{ category, products: [{ productId, discount, qty }] }] }.
// Turn that into the same { [productId]: {checked, discount} } / qty maps
// shape the localStorage hydration effect below produces, so both sources
// can be merged the same way.
const extractSelectionFromVendorProducts = (vendorProducts) => {
  const map = {};
  const qtyMap = {};
  if (!vendorProducts) return { map, qtyMap };
...
const VendorStockUpdatePage = () => {
  ...
  // Restore any products the vendor already checked/discounted last time
  // they were on this page, so the cart survives navigation/reloads. This
  // is the OVERLAY layer: it merges on top of (and, for shared product
