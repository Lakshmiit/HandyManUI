...
const AdminCollectionNotificationGrid = () => {
  ...
  useEffect(() => {
    setLoading(true);
    const url = `https://handymanapiv4-d4baa3hhdcftgabe.centralindia-01.azurewebsites.net/api/LakshmiCollection/GetAllLakshmiCollectionsOpen`
    axios.get(url)
      .then(response => {
  ...
  const handleDelete = (collectionId) => {
    ...
    if (confirmDelete) {
      axios.delete(`https://handymanapiv4-d4baa3hhdcftgabe.centralindia-01.azurewebsites.net/api/RaiseTicket/${collectionId}`)
        .then(() => {
