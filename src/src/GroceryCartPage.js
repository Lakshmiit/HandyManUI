...
useEffect(() => {
        ...
        const results = await Promise.allSettled(
          filenames.map(async (fn) => {
            ...
            if (contentType.includes("application/json")) {
              ...
              return { fn, url: `https://lmartapiv1-fxcyd2b4btacgsav.westus2-01.azurewebsites.net${encodeURIComponent(fn)}` };
            }
          }),
        );
        if (cancelled) return;
 ...
 const handleQtyChange = async (rowId, delta) => {
  ...
  try {
    const res = await fetch(
      `https://lmartapiv1-fxcyd2b4btacgsav.westus2-01.azurewebsites.net/api/UploadGrocery/GetGroceryItemsByProductName?productName=${encodeURIComponent(item.name)}`
    );
    const data = await res.json();
    const latest = data?.[0];
