...
const ProfilePage = () => {
  ...
  useEffect(() => {
    const effectiveZipCode = zipCode || "530048";
    console.log("zipCode:", zipCode, "| using:", effectiveZipCode);
    let cancelled = false;
    (async () => {
      try {
        const vendors = await getVendorsByPincode(effectiveZipCode);
        if (!cancelled) {
  ...
  //    const normalizeVendorJsonProduct = (p) => ({
  //   id: `json-${p.productId}`,
  //   name: p.productName,
  //   mrp: Number(p.mrp || 0),
  //   afterDiscount: Number(p.price || 0),
  //   discount: Number(p.discount || 0),
  //   units: p.units || "",
  //   stockLeft: Number(p.qty || 0),
  //   limit: 0,
  // });

  // const renderVendorJsonProductCard = (rawProduct) => {
  //   const product = normalizeVendorJsonProduct(rawProduct);
  //   const maxQty = getMaxAllowedQty(product);
  //   const isOutOfStock = maxQty <= 0;
  //   const placeholderImg = makePlaceholderImage(product.name, "ff5722", "ffffff");
  ...
  //         style={{ fontSize: "11px", display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical", overflow: "hidden", lineHeight: "1.2em", maxHeight: "3.6em" }}
  //       >
  //         {product.name}
  //       </h6>
  ...
  //           {product.mrp ? <s className="text-muted">₹{product.mrp}</s> : null}
  ...
  //               style={{ backgroundColor: "green", color: "white", borderRadius: "8px", padding: "2px", minWidth: "60px" }}
  //             >
  //               <button className="btn btn-sm p-0 text-white" onClick={() => handleDecrementClick(product)}>–</button>
                      ...
                      {approvedVendorListJson.map((v) => {
                        ...
                        return (
                          <button
                            ...
                            onClick={() => setSelectedMartTab(v.vendorId)}
                            className="vendor-tab"
                            style={{
                              flex: "0 0 auto",
                              border: isActive
                                ? "1.5px solid #ff5722"
                                : "1px solid #ddd",
                              background: isActive ? "#fff3ed" : "#fff",
