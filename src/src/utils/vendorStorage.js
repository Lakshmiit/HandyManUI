...
// Marks a vendor's selected locally-saved requests as Approved.
export const approveVendorProductRequests = (vendorName, ids) => {
  const requests = getVendorProductRequests();
  const updated = requests.map((r) =>
    r.requestedBy === vendorName && ids.includes(r.id) ? { ...r, status: "Approved" } : r
  );
