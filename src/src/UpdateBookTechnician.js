...
useEffect(() => {
    const fetchTechnicianData = async () => {
      try {
        const response = await fetch(`https://lmartapiv1-fxcyd2b4btacgsav.westus2-01.azurewebsites.net/api/UploadJobDescriptionBookTechnician/GetTicket/${id}`);
        if (!response.ok) {
...
const handleUpdateJobDescription = async (e) => {
    ...
    try {
      const response = await fetch(`https://lmartapiv1-fxcyd2b4btacgsav.westus2-01.azurewebsites.net/api/UploadJobDescriptionBookTechnician/${descriptionId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
