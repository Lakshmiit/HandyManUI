...
<div className={`container ${isMobile ? 'w-100' : 'w-75'}`}>
  <div className="row row-cols-3 row-cols-md-5 g-4 mt-1">
    {categories.map((cat) => (
  ...
  <div className="col" key={cat.label} onClick={() => handleCategoryClick(cat)}>
    ...
    >
      <img
        src={cat.image}
        alt={cat.label}
        style={{ height: '65px', width: '65px', borderRadius: '8px',  
          // marginTop: '5px', objectFit: 'cover'
         }}
      />
      <span style={{ fontSize: '12px', fontWeight: '500', marginTop: '0px' }}>{cat.label}</span>
    </div>
  </div>
))} */}
  </div>
