...
const RaiseActionView = () => {
  ...
  useEffect(() => {
    const fetchticketData = async () => {
      try {
        const response = await fetch(`https://lmartzoneav1-bhdzfxcse7ctdxbd.westus2-01.azurewebsites.net/api/RaiseTicket/GetTicket/${raiseTicketId}`);
        if (!response.ok) {
  ...
  const handleSaveTicket = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`https://lmartzoneav1-bhdzfxcse7ctdxbd.westus2-01.azurewebsites.net/api/RaiseTicket/${raiseTicketId}`, {
        method: 'PUT',
        headers: {
